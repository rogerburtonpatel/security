...
def xss_test(target_url, xss_payload, vulnerability_counter):
    ...
    if xss_payload in req.text:
        print("\nXSS Vulnerablity discovered!")
        print(f'{xss_payload} appended to url is potential XSS opening.')
        vulnerability_counter += 1
    else:
        print(f'Scanner indicates site is secure from payload {xss_payload}')
    ...
    if fuzz_input == '2':
      ...
      vulnerability_counter = xss_test(target, payload, vulnerability_counter)
        ...
        with io.open(fuzz_input,'r+') as f:
            ...
            # Adds payload to url and posts. If the payload is found in the 
            # output, ups the vulnerability counter and prints the offending 
            # payload. This indicates an XSS vulnerability. 
            for line in f:
                payload = line 
